...
class GluePromptOpt:
    ...
    def calculate_tokens(self, input_text: str, output_text: str) -> dict:
        """
        Cette fonction calcule le nombre de tokens d'input et d'output.

        :param input_text: Le texte d'entrée
        :param output_text: Le texte de sortie
        :return: Dictionnaire avec le nombre de tokens d'input et d'output
        """
        # Initialiser le tokenizer pour le modèle GPT (par exemple GPT-3 ou GPT-4)
        enc = tiktoken.get_encoding("gpt2")  # ou utilisez un autre modèle si nécessaire
    ...
    def predict_and_access(self, question: str, gt_answer: str) -> (bool, str, str):
        """
        Pour la question donnée, obtenir la réponse du LLM, en utilisant le BEST_PROMPT et EXPERT_PROFILE
        calculés précédemment, et calculer les tokens d'input et d'output.

        :param question: Question à poser au LLM
        :param gt_answer: Réponse correcte, la réponse finale.
        :return:  (is_correct, predicted_ans, llm_output)
                is_correct -> Indique si la prédiction par le LLM était correcte.
                predicted_ans -> La réponse prédite par le LLM.
                llm_output -> Le texte généré par le LLM pour la question donnée
        :rtype: (bool, str, str)
        """
        ...
        token_counts = self.calculate_tokens(final_prompt, llm_output)
        self.logger.info(f"Token count - Input: {token_counts['input_tokens']}, Output: {token_counts['output_tokens']}")
